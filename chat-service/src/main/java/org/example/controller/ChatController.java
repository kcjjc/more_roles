...
import org.example.service.ChatStreamAssembler;
...
public class ChatController {
    ...
    public Flux<ServerSentEvent<String>> chatStream(@PathVariable("id") Long conversationId,
                                                    @RequestBody ChatRequest req) {
        Long userId = StpUtil.getLoginIdAsLong();
        try {
            return chatService.chatStream(userId, conversationId, req.content());
        } catch (RuntimeException e) {
            return Flux.just(ChatStreamAssembler.errorFrame(e.getMessage()));
        }
    }
