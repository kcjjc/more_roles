 ...
 * 需要时查 doc_chunk, 命中片段并入 system 的【参考资料】段. 命中内容只进当轮 prompt, 不落 message 表.
 * <p>
...
public class ChatService {
    ...
    public String chat(Long userId, Long conversationId, String userContent) {
        ...
        if (conv.getKbId() != null) {
            ...
            if (rd.need()) {
                hits = retrievalService.search(rd.query(), conv.getKbId(), null);   // topK=null → 默认配置
            }
        }
     ...
     */
    private String buildSystem(String persona, String summary, List<ChunkHit> hits) {
